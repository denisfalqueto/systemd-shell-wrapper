	...
	case $1 in
		...
		enable|disable)
			if [[ "${daemon%%@*}" == "${daemon}" ]]; then # sadly is-enabled does not work as expected for "@" services like dhcpcd@eth0
				systemctl -q is-enabled "${daemon}" >& /dev/null
				if [[ $? -eq 0 ]]; then
...
s_daemon_exists() {
	unitType="${2:-service}"
	if systemctl --no-legend -t "$unitType" list-unit-files | grep -v static | grep -q "^${1%%@*}@*\.$unitType" >& /dev/null; then return 0; else return 1; fi
}
